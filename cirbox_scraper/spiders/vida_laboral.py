...
class SpiderVidaLaboral(scrapy.Spider):
    ...
    def start_requests(self):
        log_trace(self, 'SpiderVidaLaboral', 'Inicio solicitud')
        try:
            log_trace(self, 'SpiderVidaLaboral', 'Iniciando sesion')
            ...
            log_trace(self, 'SpiderVidaLaboral', f"Error en la exploración de las URLs: {str(err)}")
            print('1', str(err))

    def do_things(self, response):
        log_html(self, response, '1', 'SpiderVidaLaboral', 'not defined')

        try:
            log_trace(self, 'SpiderVidaLaboral', 'Iniciando el procesado de la respuesta')
            ...
            log_trace(self, 'SpiderVidaLaboral', 'Documento subido')

        except Exception as err:
            log_trace(self, 'SpiderVidaLaboral', f"Error en el proceso: {str(err)}")
            raise err
